...
--for cam_num = 1, 2 do
  ...
  local cam_dir = dataset_dir .. 'bbox_train/'
  --print(cam_dir)
  lfs.mkdir(write_dir .. 'bbox_train')
  for person in lfs.dir(cam_dir) do
    if person~="." and person~=".." and skip~=1 then
      local dir_path = cam_dir .. person
     -- print(dir_path)
      lfs.mkdir(write_dir .. 'bbox_train' .. '/' .. person)
      ...
      for file in lfs.dir(dir_path) do
          ...
          if file~="." and file~=".." and file~=".DS_Store" then
              ...
              torch.save(write_dir .. 'bbox_train' .. '/' .. person .. '/' .. file .. '_features.t7', {features=features, image_list=list_of_filenames})
          end
      end
      print('saved features for ' .. 1 .. ' ' .. person)
    end
  end
--end
